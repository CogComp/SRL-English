...
@Predictor.register("noun-semantic-role-labeling")
class NounSemanticRoleLabelerPredictor(Predictor):
    """
    ...
    def predict_tokenized(self, tokenized_sentence: List[str]) -> JsonDict:
        ...
        A dictionary representation of the nominal semantic roles of the sentence.
        """
        print('PREDICT_TOKENIZED-----------')
        new_sentence, _ = separate_hyphens(" ".join(tokenized_sentence)) # just make sure hyphens separated out
        spacy_doc = Doc(self._tokenizer.spacy.vocab, words=new_sentence)
    ...
    def tokens_to_instances(self, tokens):
        ...
        tokens: `List[Token]`, required
            List of tokens (already tokenized and hyphens separated).
        """
        words = [token.text for token in tokens]
        instances: List[Instance] = []
        for i, word in enumerate(tokens):
            if word.pos_ == "NOUN":
                nom_labels = [0 for _ in tokens]
                nom_labels[i] = 1
                instance = self._dataset_reader.text_to_instance(tokens, tokens, nom_labels)
    ...
    def _sentence_to_srl_instances(self, json_dict: JsonDict) -> List[Instance]:
        ...
        new_sentence, _ = separate_hyphens(sentence.split())
        tokens = self._tokenizer.split_words(" ".join(new_sentence))
        ...
        for sentence_index, nom_count in enumerate(noms_per_sentence):
            ...
            for _ in range(nom_count):
                ...
                return_dicts[sentence_index]["nominals"].append(
                        {"nominal": output["nominal"], "predicate_index": output["nominal_indices"], "description": description, "tags": tags}
                        )
                output_index += 1
