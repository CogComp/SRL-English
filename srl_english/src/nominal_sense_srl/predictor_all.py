...
@Predictor.register("all-nombank-sense-srl")
class AllNomSenseSRLPredictor(Predictor):
    """
    ...
    def predict_tokenized(self, tokenized_sentence: List[str]) -> JsonDict:
        ...
        A dictionary representation of the nominal semantic roles of the sentence.
        """
        new_sentence, _ = separate_hyphens(words)
    ...
    def tokens_to_instances(self, tokens):
        ...
        for i, word in enumerate(tokens):
            if word.pos_ == "NOUN":
                noun_labels = [0 for _ in words]
                noun_labels[i] = 1
                instance = self._dataset_reader.text_to_instance(tokens, tokens, noun_labels)
