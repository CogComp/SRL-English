...
@Model.register("combined-srl-model")
class CombinedSRLModel(Model):
    """
    ...
    def __init__(self,
        ...
        super(CombinedSRLModel, self).__init__(vocab, regularizer)
    ...
    def forward(self, # type: ignore
                ...
                predicate_indicator: torch.Tensor,
                metadata: List[Any],
        ...
        predicate_indicator: torch.LongTensor, required.
        ...
        metadata : ``List[Dict[str, Any]]``, optional, (default = None)
            metadata containg the original words in the sentence, the predicate to compute the
            frame for, and start offsets for converting wordpieces back to a sequence of words,
            under 'words', 'predicate' and 'offsets' keys, respectively.
        Returns
        -------
        ...
        # predicate_idx = torch.nonzero(predicate_indicator)
        # torch.nonzero gives us all indices of nonzero elts, so just get the first one.
        output_dict = {"logits": tag_logits, "class_probabilities": class_probabilities}
        # We need to retain the mask in the output dictionary
        # so that we can crop the sequences to remove padding
        # when we do viterbi inference in self.decode.
        output_dict["mask"] = mask
        # We add in the offsets here so we can compute the un-wordpieced tags.
        words, predicates, offsets = zip(*[(x["words"], x["predicate"], x["offsets"]) for x in metadata])
        output_dict["words"] = list(words)
        output_dict["predicate"] = list(predicates)
        output_dict["wordpiece_offsets"] = list(offsets)
        if tags is not None:
            loss = sequence_cross_entropy_with_logits(tag_logits,
                                                      tags,
                                                      mask,
                                                      label_smoothing=self._label_smoothing)
            if not self.ignore_span_metric and self.span_metric is not None and not self.training:
                batch_predicate_indices = [example_metadata["predicate_index"][0] if isinstance(example_metadata["predicate_index"], list) else example_metadata["predicate_index"] for example_metadata in metadata]
                ...
                self.span_metric(batch_predicate_indices,
                                 batch_sentences,
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        """
        tag_predictions = output_dict['class_probabilities']
