...
from combined_unconstrained_srl.reader import separate_hyphens

@Predictor.register("combined-semantic-role-labeling")
class CombinedSRLPredictor(Predictor):
    """
    ...
    def predict_tokenized(self, tokenized_sentence: List[str]) -> JsonDict:
        ...
        if not instances:
            return sanitize({"predicates": [], "words": tokens})
    ...
    def tokens_to_instances(self, tokens):
        ...
        for i, word in enumerate(tokens):
            if word.pos_ == "NOUN" or word.pos_ == "VERB" or word.pos_ == "ADJ":
                new_predicate_idx = new_indices[i]
                predicate_labels = [0 for _ in new_tokens]
                for new_i in new_predicate_idx:
                    predicate_labels[new_i] = 1
                instance = self._dataset_reader.text_to_instance(new_tokens, predicate_labels)
    ...
    def _sentence_to_srl_instances(self, json_dict: JsonDict) -> List[Instance]:
        ...
        forward for every predicate in the sentence. This means that for a single sentence, we need
        to generate a ``List[Instance]``, where the length of this list corresponds to the number
        of predicates in the sentence. Additionally, all of these predicates share the same return dictionary
    ...
    def predict_batch_json(self, inputs: List[JsonDict]) -> List[JsonDict]:
        ...
        if not flattened_instances:
            return sanitize([{"predicates": [], "words": self._tokenizer.split_words(x["sentence"])}
                             for x in inputs])
        ...
        predicates_per_sentence = [len(sent) for sent in instances_per_sentence]
        return_dicts: List[JsonDict] = [{"predicates": []} for x in inputs]

        output_index = 0
        for sentence_index, predicate_count in enumerate(predicates_per_sentence):
            if predicate_count == 0:
                # We didn't run any predictions for sentences with no predicates,
            ...
            for _ in range(predicate_count):
                ...
                return_dicts[sentence_index]["predicates"].append({
    ...
    def predict_instances(self, instances: List[Instance]) -> JsonDict:
        ...
        results = {"predicates": [], "words": outputs[0]["predicates"]}
    ...
    def predict_json(self, inputs: JsonDict) -> JsonDict:
        ...
        if not instances:
            return sanitize({"predicates": [], "words": self._tokenizer.split_words(inputs["sentence"])})
