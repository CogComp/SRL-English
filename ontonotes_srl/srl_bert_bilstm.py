...
import torch
from torch.nn.modules import Linear, Dropout, Embedding, Sigmoid, Sequential, LSTM
...
@Model.register("srl_bert_bilstm")
class SrlBertBilstm(Model):
    """
    ...
    def __init__(self,
                 ...
                 recurrent_hidden_layer_size: int,
                 mlp_hidden_size: int,
                 predicate_emb_dim: int,
                 lstm_combo_mode: str = "concatenate",
        ...
        self.predicate_embedder = Embedding(2, predicate_emb_dim)
        self.recurrent_layer = LSTM(self.bert_model.config.hidden_size + predicate_emb_dim, recurrent_hidden_layer_size, batch_first=True, bidirectional=True)
        self.lstm_combo_mode = lstm_combo_mode
        self.mlp_hidden_size = mlp_hidden_size
        if self.lstm_combo_mode == "concatenate":
            linear1 = Linear(2*recurrent_hidden_layer_size, mlp_hidden_size)
        elif self.lstm_combo_mode == "add":
            linear1 = Linear(recurrent_hidden_layer_size, mlp_hidden_size)
        else:
            raise ConfigurationError("lstm_combo_mode must be either concatenate or add.")
        linear2 = Linear(mlp_hidden_size, self.num_classes)
        sigmoid = Sigmoid()
        self.tag_projection_layer = Sequential(linear1, sigmoid, linear2)
    ...
    def forward(self, # type: ignore
        ...
        embedded_text_input = torch.cat([self.embedding_dropout(bert_embeddings), predicate_embedding], 2)
        batch_size, sequence_length, _ = embedded_text_input.size()
        recurrent_output, (hidden_text, _) = self.recurrent_layer(embedded_text_input)
        ...
        if self.lstm_combo_mode == "concatenate":
            ...
            recurrent_combined = fwd_output+back_output
        logits = self.tag_projection_layer(recurrent_combined)
