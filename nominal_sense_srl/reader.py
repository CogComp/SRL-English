...
def get_bio_tags(args: List[str], new_indices: List[List[int]], new_sentence: List[str]):
    ...
    print('ALL ARGS ORDERED: ', all_args_ordered)
    filtered_args_ordered = []
    # loop to get rid of overlapped spans
    prev_arg = None
    for idx, arg in enumerate(all_args_ordered):
        if not prev_arg:
            prev_arg = arg
            continue
        if arg[0] <= prev_arg[1]: # overlap exists
            ...
            prev_arg = arg
    filtered_args_ordered.append(prev_arg)
    print('FITERED ARGS ORDERED: ', filtered_args_ordered)
...
class NombankSenseSRLReader(DatasetReader): 
    ...
    def _read(self, file_path: str):
        ...
        for (og_sentence, og_nom_loc, new_indices, new_sentence, new_tags, sense) in srl_data:
            ...
            yield self.text_to_instance(og_tokens, new_tokens, nom_indicator, new_tags, sense)
    ...
    def read_nom_srl(self, filename):
        ...
        for line in fin.readlines():
            ...
            data.append((og_sentence, og_nom_loc, new_indices, new_sentence, new_tags, sense))
        fin.close()
        return data
    ...
    def text_to_instance(
        ...
        """
        # print('TEXT TO INSTANCE', new_tokens, ', ', nom_label, ',',  new_tags, ',', sense)
